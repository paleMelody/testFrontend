...
/**
 * ============================================================
 * 左侧弧形面板组件（LeftArcPanel）
 *
 * 本组件是从 ArcPanel 拆分出来的独立左弧组件，弧形方向固定为左侧。
 * 右侧对应组件请参见 RightArcPanel.vue。
 *
 * ────────────────────────────────────────────────────────────
 * 【如何调整左侧弧度】
 *
 ...
 *
 *   示例：
 *     <LeftArcPanel :curvature="0.7" :hours="leftHours" @markClick="goTo" />
 *
...
// 使用 composable 计算刻度位置和弧形路径（固定 side = 'left'）
const { panelW, panelH, marks, arcPath } = useArcPanel(hoursRef, 'left', wrapEl, curvatureRef)
  ...
  <div class="arc-panel-wrap" ref="wrapEl">
    ...
    <template v-if="showDots">
      <div
        ...
        :style="{ top: m.cy + 'px', left: (m.cx + 10) + 'px' }"
        @click="emit('markClick', m.hour)"
      >{{ fmtHour(m.hour) }}</div>
    </template>
  </div>
